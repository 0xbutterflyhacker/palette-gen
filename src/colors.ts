...
export class color {
  ...
  constructor(r: number, g: number, b: number) {
    ...
    this.hex = `#${r > 15 ? r.toString(16) : '0' + r.toString(16)}${
   ...
   */
  toHSL(): number[] {
    const sol: number[] = Array.from({ length: 3 });
    const rp = this.red / 255;
    const gp = this.green / 255;
    const bp = this.blue / 255;
   ...
   */
  triadic(): color[] {
    const mid: number[][] = Array.from({ length: 2 }).map(() => this.toHSL());
    mid[0][0] -= 120;
    if (mid[0][0] < 0) mid[0][0] += 360;
    mid[1][0] += 120;
    if (mid[1][0] > 359) mid[1][0] %= 360;
    const sol: color[] = mid.map(toRGB);
    return sol;
  }
  /**
...
export function toRGB(l: number[]): color {
  const c: number = (1 - Math.abs(2 * l[2] - 1)) * l[1];
  const x: number = c * (1 - Math.abs(((l[0] / 60) % 2) - 1));
  const m: number = l[2] - c / 2;
  let rp: number = 0;
  let gp: number = 0;
  let bp: number = 0;
  if (0 <= l[0] && l[0] < 60) {
